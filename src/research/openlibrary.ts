// Open Library provider: books as "go deeper" doorways. The card text is the
// book's own first sentence when the catalog has one — still verbatim.
...
export async function searchOpenLibrary(query: string, max = 2): Promise<{ docs: SourceDoc[]; passages: Passage[] }> {
  const data = (await getJSON(
    `https://openlibrary.org/search.json?q=${encodeURIComponent(query)}&limit=10&fields=title,author_name,first_publish_year,key,first_sentence`,
  )) as { docs?: OlDoc[] } | null;
  ...
  for (const b of found) {
    ...
    const doc: SourceDoc = {
      ...
      ...(b.author_name?.[0] ? { author: b.author_name[0] } : {}),
      ...(b.first_publish_year ? { date: String(b.first_publish_year) } : {}),
    };
    docs.push(doc);
    passages.push({
      id: freshId('p'),

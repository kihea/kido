...
// own verbatim words about their work, and the DOI leads to the full paper.
...
export async function searchCrossref(query: string, rows = 6): Promise<{ docs: SourceDoc[]; passages: Passage[] }> {
  ...
  for (const item of items) {
    ...
    // Lead with the finding, not "This paper examines…". If the whole thing is
    // preface, keep it — a paper card is still a doorway to the full work.
    const abstract = dropLeadingMeta(raw, 240) || raw;
    if (abstract.length < 200) continue;
    const first = item.author?.[0];
    const author = first
      ? [first.given, first.family].filter(Boolean).join(' ') + ((item.author?.length ?? 0) > 1 ? ' et al.' : '')
      : undefined;
